...
const combinatorial = algorithms.flatMap((algorithm) => signatureTypes.flatMap((signatureType) => opcodePatterns.flatMap((pattern) => [true, false].map((valid) => [algorithm, signatureType, pattern, valid])))).filter(([_, signatureType, pattern]) => pattern[0].includes(signatureType[0]));
...
//# sourceMappingURL=core.signing-serialization.js.map