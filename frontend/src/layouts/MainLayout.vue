    ...
    <q-layout view="lHh Lpr lFf">
          ...
          <div class="header-right">
            ...
            >
              ...
              <span style="display: flex; flex-direction: column; line-height: 1.2">
                <span
                  style="font-size: 8px; opacity: 0.8; font-weight: 700; letter-spacing: 0.05em"
                >
            ...
            <button v-else class="wallet-pill" type="button" @click="onConnectWallet">
              ...
              <span style="display: flex; flex-direction: column; line-height: 1.2">
                ...
                <span class="wallet-addr text-mono" style="font-size: 11px"> CONNECT WALLET </span>
              </span>
            </button>
            <button
              class="btn--icon"
      ...
      <q-footer>
        <div class="container footer-content">
          <div class="footer-left">&copy; 2026 HODLVAULT WEB APP</div>
          <div class="footer-right">
            <span class="status-item">SYSTEM VERSION: v4.0.1</span>
            <span
              class="status-item"
              :class="
                networkLabel === 'CHIPNET'
                  ? 'text-neon'
                  : networkLabel === 'MAINNET'
                    ? 'text-green'
                    : 'text-grey'
              "
              >NETWORK: {{ networkLabel }}</span
            >
          </div>
        </div>
      </q-footer>
    ...
    <q-dialog v-model="showNotificationModal">
      <q-card style="max-width: 420px; width: 100%">
        <q-card-section class="modal-header">
          <h3 style="margin: 0; font-size: 16px; font-family: var(--font-heading)">
        ...
        <q-card-section class="modal-body">
          ...
          <div class="setting-item">
            ...
            <label
              class="toggle-switch"
              :class="{
                'toggle-switch--disabled': !connectedAddress || notificationPermissionDenied,
              }"
            >
              <input
                type="checkbox"
                v-model="notificationsEnabled"
                :disabled="!connectedAddress || notificationPermissionDenied"
                @change="toggleNotifications(notificationsEnabled)"
              />
          ...
          <div
            ...
            class="text-muted"
            style="font-size: 10px; display: flex; align-items: center; gap: 4px; margin-top: -8px"
          >
            <i class="material-icons" style="font-size: 12px; color: var(--color-warning)"
              >warning</i
            >
          ...
          <div
            class="setting-item"
            style="flex-direction: column; align-items: stretch; gap: 12px; margin-top: 8px"
          >
            <div
              style="
                display: flex;
            ...
            >
              ...
              <label
                class="toggle-switch"
                :class="{ 'toggle-switch--disabled': !connectedAddress }"
              >
                <input
                  type="checkbox"
                  v-model="emailNotificationsEnabled"
                  :disabled="!connectedAddress || !savedEmail"
                  @change="toggleEmailNotifications(emailNotificationsEnabled)"
                />
                <span class="toggle-slider"></span>
              </label>
            </div>
            ...
            <template v-if="savedEmail">
              <div style="display: flex; align-items: center; gap: 8px; padding: 8px 0">
                ...
                <span
                  ...
                  >VERIFIED</span
                >
                <span v-else style="font-size: 10px; color: var(--color-warning); font-weight: 700"
                  >UNVERIFIED</span
                >
                <button
                  style="
              ...
              <div
                v-if="!emailVerified && showVerifyCodeInput"
              ...
              <div style="display: flex; gap: 8px">
                <input
                  ...
                  style="
                    ...
                    outline: none;
                  "
                  :disabled="!connectedAddress || savingEmail"
                  @keyup.enter="saveEmail"
                />
                <button
                  class="btn btn--outline"
                  style="padding: 8px 16px; font-size: 12px"
                  :disabled="!emailInput || !connectedAddress || savingEmail"
                  @click="saveEmail"
                >
...
export default defineComponent({
  ...
  methods: {
    ...
    async loadNotificationPreferences() {
      if (!this.connectedAddress) {
        console.log(
          '[NotifDebug:MainLayout] loadNotificationPreferences skipped — no connectedAddress',
        )
        return
      }
      console.log(
        `[NotifDebug:MainLayout] loadNotificationPreferences() called | address=${this.connectedAddress.slice(0, 16)}...`,
      )
      try {
        ...
        console.log(
          `[NotifDebug:MainLayout] getPreferences response | email=${prefs?.email ? prefs.email.slice(0, 6) + '...' : 'null'} | emailVerified=${prefs?.emailVerified} | emailNotifications=${prefs?.preferences?.emailNotifications} | notifications=${prefs?.preferences?.notifications}`,
        )
        ...
        console.log(
          `[NotifDebug:MainLayout] loadNotificationPreferences done | savedEmail=${this.savedEmail ? this.savedEmail.slice(0, 6) + '...' : 'null'} | emailVerified=${this.emailVerified} | emailNotif=${this.emailNotificationsEnabled} | showVerify=${this.showVerifyCodeInput}`,
        )
      } catch (err) {
    ...
    async toggleEmailNotifications(enabled) {
      console.log(
        `[NotifDebug:MainLayout] toggleEmailNotifications(${enabled}) | savedEmail=${!!this.savedEmail} | emailVerified=${this.emailVerified}`,
      )
      try {
        ...
        if (enabled && !this.emailVerified) {
          console.log(
            '[NotifDebug:MainLayout] toggleEmailNotifications blocked — email not verified',
          )
          this.emailNotificationsEnabled = false
        ...
        console.error(
          `[NotifDebug:MainLayout] toggleEmailNotifications failed | error=${err.message}`,
          err,
        )
        this.emailNotificationsEnabled = !enabled
        this.$q.notify({
    ...
    async saveEmail() {
      ...
      console.log(
        `[NotifDebug:MainLayout] saveEmail() called | email=${this.emailInput.slice(0, 6)}...`,
      )

      const typo = suggestDomainFix(this.emailInput)
      if (typo) {
        console.log(
          `[NotifDebug:MainLayout] Domain typo detected | typed=${typo.typed} | suggested=${typo.suggestion}`,
        )
      ...
      try {
        ...
        console.log(
          `[NotifDebug:MainLayout] saveEmail response | verificationSent=${result.verificationSent} | verificationError=${result.verificationError || 'none'} | email=${result.email ? result.email.slice(0, 6) + '...' : 'null'}`,
        )
        ...
        if (result.verificationSent) {
          ...
          this.$q.notify({
            type: 'warning',
            message:
              'Email saved but verification email could not be sent. Check SMTP settings or click Resend.',
            timeout: 8000,
          })
        }
      } catch (err) {
        ...
        console.error(
          `[NotifDebug:MainLayout] saveEmail failed | error=${errMsg} | responseStatus=${err?.response?.status || 'N/A'}`,
          err,
        )
        this.$q.notify({
          type: 'negative',
        ...
        console.log(
          `[NotifDebug:MainLayout] saveEmail() complete | savingEmail=false | savedEmail now=${this.savedEmail ? this.savedEmail.slice(0, 6) + '...' : 'null'}`,
        )
      }
    },
    ...
    async verifyCode() {
      ...
      try {
        ...
        console.log(
          `[NotifDebug:MainLayout] verifyCode() complete | emailVerified=${this.emailVerified}`,
        )
      }
    },
